//
// Created by onter on 18-9-5.
//

#ifndef PROJECT_TUNNEL_H
#define PROJECT_TUNNEL_H
...
private:
    ...
    void onClientConnection(const muduo::net::TcpConnectionPtr& conn)
    {
        ...
        if (conn->connected())
        {
            ...
            conn->setHighWaterMarkCallback(boost::bind(&Tunnel::Tunnel::onHighWaterMarkWeak,
                                           boost::weak_ptr<Tunnel>(shared_from_this()), kClient, _1, _2), 1024*1024);
                         ...
                         size_t bytesToSent)
    {
        ...
        if (which == kServer)
        {
            if (serverConn_->outputBuffer()->readableBytes() > 0)
            {
                ...
                serverConn_->setWriteCompleteCallback(boost::bind(&Tunnel::onWriteCompleteWeak,
    ...
    static void onWriteCompleteWeak(const boost::weak_ptr<Tunnel>& wkTunnel,
                                    ...
                                    const muduo::net::TcpConnectionPtr& conn)
    {
        boost::shared_ptr<Tunnel> tunnel = wkTunnel.lock();   //判断是否可以提升，　防止在析构后调用。
        if (tunnel)
        {
