...
#include "sha.h"
/*
* 将unit8 b[i] ~b[i+3] 按大端存储转化成unit32 n
*/
...
/*
* 将 unit32 n按大端存储转化成unit8 b[i] ~b[i+3]
*/
...
void sha1_process(struct sha1_contxt* ctx, uint8 data[64])
{
...
/**
 * 填充数据 如果填充后数据超过或者512 则进行摘要，否则进行将数据填充
 * @ctx 摘要信息
 * @input 输入数据
 * @length 输入数据长度
 */
...
{
    uint32 left, fill;  //均是指字节
    if (! length)
        return;

    left = (ctx->total[0] >> 3) & 0x3F; // total[0]存放的bit,要转化成字节，每次只处理512bit，所以＆0x3F
    fill = 64 - left;

    ctx->total[0] += length << 3;   // length存放的是strlen的结果，所以要转化成字节
    ctx->total[1] += length >> 29;

    ctx->total[0] &= 0xFFFFFFFF;
    ctx->total[1] += ctx->total[0] < (length << 3);
    if (left && length >= fill)          //用于刚好填充完毕的情况
    {
...
void sha1_finish(struct sha1_contxt * ctx, uint8 digest[20])
{
    uint32 last, padn;
    uint8 msglen[8];
    PUT_UINT32(ctx->total[1], msglen, 0);    //获得处理数据的长度
    PUT_UINT32(ctx->total[0], msglen, 4);

    last = (ctx->total[0] >> 3) & 0x3F;
    padn = (last < 56) ? (56 - last) : (120 - last);

    sha1_update(ctx, sha1_padding, padn);    //补零
    sha1_update(ctx, msglen, 8);             //补长度并进行摘要
...
unsigned char* Sha(const char * plaintext)
{
    if (plaintext == nullptr)
        return nullptr;
    unsigned char *ciphertext = new unsigned char[20];
    int len = strlen((const char*)plaintext);

    sha1_contxt sh;
    sha1_starts(&sh);

    sha1_update(&sh, (unsigned char *)plaintext, len);
