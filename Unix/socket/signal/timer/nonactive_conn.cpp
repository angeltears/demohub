  ...
  while (!stop_server)
  {
    ...
    for (int i = 0; i < number; i++)
    {
      ...
      else if (events[i].events & EPOLLIN)
      {
        ...
        ret = recv(sockfd, user[sockfd].buff, BUFFER_SIZE, 0);
        printf("get %d bytes from sockfd %d\n", ret, sockfd);

        util_time *timer = user[sockfd].time;
        if (ret < 0)
        {
          if (errno != EAGAIN)
          {
        ...
        else
        {
          ...
          if(timer)
          {
            ...
            printf("adjust tiemr sockfd[%d] once\n", sockfd);
